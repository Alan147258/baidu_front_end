...
var Show = {
    ...
    showPlaneInf: function(plane){
        var table = $('#table');
        if(plane.order =="ready"){
            var tr = "<tr><td>"+plane.planeName+"</td><td>"+plane.powerSystem+
                "</td><td>"+plane.energySystem+"</td><td>"+plane.condition+
                "</td><td>"+plane.energy+"</td></tr>";
            table.append(tr);
        }else if(plane.order =="start"){
            $("td:contains("+plane.planeName+")").parent()[0].innerHTML=
                "<td>"+plane.planeName+"</td><td>"+plane.powerSystem+
            "</td><td>"+plane.energySystem+"</td><td>"+plane.condition+
            "</td><td>"+parseInt(plane.energy)+"%"+"</td>";
        }else if(plane.order =="suspend"){
            $("td:contains("+plane.planeName+")").parent()[0].innerHTML=
                "<td>"+plane.planeName+"</td><td>"+plane.powerSystem+
                "</td><td>"+plane.energySystem+"</td><td>"+plane.condition+
                "</td><td>"+parseInt(plane.energy)+"%"+"</td>";
        }else if(plane.order =="destroy"){
            $("td:contains("+plane.planeName+")").parent().remove();
        }
